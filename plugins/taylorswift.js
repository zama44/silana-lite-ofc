...
let handler = async (m, { conn, text, prefix, command }) => {
    ...
    try {
        const response = await axios.get(`https://api.agatz.xyz/api/voiceover?text=${encodeURIComponent(text)}&model=taylor_swift`);
...
handler.help = ['taylorswift'];
handler.tags = ['ai'];
handler.command = ['taylorswift'];
handler.limit = true 
